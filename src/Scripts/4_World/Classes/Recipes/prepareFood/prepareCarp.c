modded class PrepareCarp
{
	...
	override void Do(ItemBase ingredients[], PlayerBase player, array<ItemBase> results, float specialty_weight)//gets called upon recipe's completion
	{
		...
		ItemBase deadCarp = ingredients[0];
		
		int filletCount = 2;
		for (int i=0; i < filletCount; i++)
		{
			ItemBase result = ItemBase.Cast(player.SpawnEntityOnGroundOnCursorDir("CarpFilletMeat", DEFAULT_SPAWN_DISTANCE));
			MiscGameplayFunctions.TransferItemProperties(deadCarp, result);
